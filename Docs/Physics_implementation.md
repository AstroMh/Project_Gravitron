# Physics Implementation Theory — Vortex Orbit–Dive Robot

This document explains the **physics theory** behind the impact-and-recovery behavior implemented in the simulator. It’s written so a reader can understand (and re-implement) the same mechanics in any codebase.
...
$$
\mathbf{F}_r = m[-k_{pr}(r - R) - k_{dr}\,v_r]\mathbf{e}_r
$$

**Tangential P (maintain tangential speed $v_{t,\mathrm{des}}$)**

$$
\mathbf{F}_t = m[-k_{pt}(v_t - v_{t,\mathrm{des}})]\mathbf{e}_t
$$
...
**Friction (tangent) with Coulomb clamp:**  
Let $\mathbf{t}=(-n_y, n_x)$ and tangential relative speed $ v_t = \mathbf{v}_{rel}\cdot\mathbf{t} $.
...
**Positional correction** (to remove overlap / prevent sticking).  
If penetration depth is
...
Typical values: $\beta\approx 0.2$, $\text{slop}\approx 0.01$.
...
Let $\mathbf{r}_c$ be the lever arm from center to contact on body A.  
With moment of inertia $I_A$ (disk: $I = \tfrac12 m r^2$), the **angular velocity** update is:
...
Even though the robot’s **angle** (orbital phase) changes every frame, treating impacts with **linear impulses** is correct and efficient.  
The change in **linear momentum** instantly updates the **angular momentum about the target**, giving natural “throw-and-recover” behavior.  
The controller then re-establishes the desired circular motion without teleportation.
...
- Game Physics (symplectic Euler, impulses): common in 2D engines (Box2D / Chipmunk)  
